...
@implementation Q1Layer

@synthesize levelSideBar, thisLevelScore;

+(id) scene
{
    CCScene *scene = [CCScene node];
    Q1Layer *layer = [Q1Layer node];
...
-(void) nextQuestion {
    ...
    newSingleton.singletonNumber = thisLevelScore;
    [[CCDirector sharedDirector] replaceScene:[CCTransitionSplitRows transitionWithDuration:1.0 scene:[Q2Layer scene]]];
}

-(void) sameQuestion {
    [[CCDirector sharedDirector] replaceScene:[CCTransitionFadeDown transitionWithDuration:1.0 scene:[Q1Layer scene]]];
}
...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        CCMenuItemFont *levelNumber = [CCMenuItemFont itemWithLabel:[CCLabelBMFont labelWithString:@"10" fntFile:@"score.fnt"] target:self selector:@selector(nextQuestion)];
        ...
        CCMenuItemFont *scoreNumber = [CCMenuItemFont itemWithLabel:[CCLabelBMFont labelWithString:@"23574" fntFile:@"score.fnt"] target:self selector:@selector(nextQuestion)];
        ...
        //Question1
        CCMenuItemFont *question1 = [CCMenuItemFont itemWithLabel:[CCLabelBMFont labelWithString:@"How many Brother\nSister You Have?" fntFile:@"title2lines.fnt"] target:self selector:@selector(nextQuestion)];
        question1.anchorPoint = ccp(1,1);
        question1.position = ccp(235,130);
